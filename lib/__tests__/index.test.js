...
const initArgList = [
  ...
  'Init Argument 2',
  ['Init', 'Argument', '3'],
  {a: 'Init', b: 'Argument', c: '4'},
];

const client = rpc(__dirname + '/../test-worker.js', ...initArgList);
test('it should work', () => {
  for (let i = 0; i < 10; i++) {
...
rpc.FUNCTION_PRIORITY.forEach((fn) => {
  test('multiple init arguments ' + fn.name, () => {
    try {
      rpc.configuration.fastestFunction = fn;
      const res = 'sent init arguments ' + JSON.stringify(initArgList);
      expect(client('getInitArgs')).toBe(res);
    } catch (ex) {
      if (fn.name === 'nativeNC') {
        console.log(fn.name + ' fails');
        return;
...
rpc.FUNCTION_PRIORITY.forEach((fn) => {
  test('multiple call arguments ' + fn.name, () => {
    try {
      rpc.configuration.fastestFunction = fn;
      const argList = [
        'My Message',
        'Call Argument 2',
        ['Call', 'Argument', '3'],
        {a: 'Call', b: 'Argument', c: '4'},
      ];
      const res = 'sent arguments ' + JSON.stringify(argList) + ' to My Server';
      expect(client.apply(null, argList)).toBe(res);
    } catch (ex) {
      if (fn.name === 'nativeNC') {
        console.log(fn.name + ' fails');
        return;
